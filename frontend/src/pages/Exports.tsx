...
export function Exports() {
  ...
  return <section className="page-section"><div className="section-heading"><div><span className="eyebrow">{t('exports')} / 05</span><h2>{t('latestExport')}</h2><p>{t('exportDescription')}</p></div>{value && <StatusBadge status={value.status} />}</div>
    ...
    {value && <div className="safe-summary"><p className="muted-text">{t('downloadUnavailable')}</p><details><summary>{t('technicalDetails')}</summary><dl><div><dt>{t('runId')}</dt><dd><code>{value.run_id}</code></dd></div><div><dt>{t('sha256')}</dt><dd><code>{value.dataset_sha256 ? `${value.dataset_sha256.slice(0, 12)}…${value.dataset_sha256.slice(-8)}` : t('notAvailable')}</code></dd></div></dl></details></div>}
  </section>;
}
