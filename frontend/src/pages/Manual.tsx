...
export function Manual() {
  ...
  return <section className="page-section manual-page"><div className="section-heading"><div><span className="eyebrow">{t('manualEyebrow')}</span><h2>{t('manualPreviewTitle')}</h2><p>{t('manualDescription')}</p></div></div>
    ...
    {message && <div className="job-panel" role="status">{message} <Link to="/sources">{t('external')}</Link></div>}
    {preview && <article className="preview-card" aria-labelledby="preview-title"><div className="preview-header"><div><span className="eyebrow">{t('temporaryPreview')}</span><h3 id="preview-title">{preview.title || t('noTitle')}</h3></div><strong>{t(LABELS[preview.disposition])}</strong></div><code dir="ltr">{preview.display_url}</code><dl className="preview-details"><div><dt>{t('pageType')}</dt><dd>{preview.page_type}</dd></div><div><dt>{t('expiresAt')}</dt><dd>{dateTime(preview.expires_at)}</dd></div></dl>{preview.detected_type === 'json_collection' && <section className="job-panel" aria-label="JSON collection mapping"><strong>JSON collection</strong><span>Collection: {preview.collection_path?.join('.') || '(root array)'}</span><dl className="preview-details">{Object.entries(preview.proposed_mapping || {}).map(([key,value]) => <div key={key}><dt>{key.replace('_field','')}</dt><dd><code>{value}</code></dd></div>)}</dl><span>Explicit approval is required. Raw JSON is never displayed.</span></section>}<dl className="job-counts">{Object.entries(preview.counts).map(([key, value]) => <div key={key}><dt>{t(COUNT_LABELS[key as keyof typeof COUNT_LABELS])}</dt><dd>{number(value)}</dd></div>)}</dl>
    ...
    {job && <JobMonitor sourceId="manual-preview" job={job} onUpdate={(_key, value) => {setJob(value);if(TERMINAL_STATES.includes(value.state))void queryClient.invalidateQueries({queryKey:externalQueryKeys.sources()});}} />}
  </section>;
}
