...
export function Manual() {
  ...
  return <section className="page-section manual-page"><div className="section-heading"><div><span className="eyebrow">المصادر / 03</span><h2>معاينة رابط يدوي</h2><p>راجع نسخة مؤقتة ومنقّاة قبل الحفظ أو التصدير.</p></div></div>
    ...
    {preview && <article className="preview-card" aria-labelledby="preview-title"><div className="preview-header"><div><span className="eyebrow">معاينة مؤقتة</span><h3 id="preview-title">{preview.title || 'دون عنوان'}</h3></div><strong>{LABELS[preview.disposition]}</strong></div><code dir="ltr">{preview.display_url}</code>{preview.excerpt && <p className="preview-excerpt">{preview.excerpt}</p>}<dl className="preview-details"><div><dt>نوع الصفحة</dt><dd>{preview.page_type}</dd></div><div><dt>التصنيف</dt><dd>{preview.classification_label || 'غير محدد'}{preview.classification_confidence !== undefined ? ` (${Math.round(preview.classification_confidence * 100)}٪)` : ''}</dd></div><div><dt>الخصوصية</dt><dd>{preview.privacy_status === 'reviewed' ? 'مراجعة مكتملة' : 'تتطلب مراجعة'}</dd></div><div><dt>تنتهي</dt><dd>{new Date(preview.expires_at).toLocaleString('ar')}</dd></div></dl>{preview.review_reasons.length > 0 && <p>أسباب المراجعة: {preview.review_reasons.join('، ')}</p>}<dl className="job-counts">{Object.entries(preview.counts).map(([key, value]) => <div key={key}><dt>{COUNT_LABELS[key as keyof typeof COUNT_LABELS]}</dt><dd>{value}</dd></div>)}</dl><div className="preview-actions"><button className="button" type="button" disabled={busy} onClick={approvePreview}>{approve.isPending ? 'جار الاعتماد...' : 'اعتماد وحفظ'}</button><label>سبب التجاهل<select value={reason} disabled={busy} onChange={(event) => setReason(event.target.value as keyof typeof REASONS)}>{Object.entries(REASONS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><button className="button button-secondary" type="button" disabled={busy} onClick={() => preview && reject.mutate()}>{reject.isPending ? 'جار التجاهل...' : 'تجاهل'}</button></div></article>}
