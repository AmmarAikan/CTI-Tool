...
export function Sources() {
  ...
  const importJob=useMutation({mutationFn:({jobId}:{sourceId:string;jobId:string})=>api.importExternalJob(jobId),onSuccess:(value,request)=>{setCentralRuns(current=>({...current,[request.sourceId]:value.run_id}));setImportErrors(current=>{const next={...current};delete next[request.sourceId];return next});void queryClient.invalidateQueries({queryKey:['external','accepted-records']});void queryClient.invalidateQueries({queryKey:externalQueryKeys.reviews()});void queryClient.invalidateQueries({queryKey:externalQueryKeys.processingRun(value.run_id)});},onError:(_error,request)=>setImportErrors(current=>({...current,[request.sourceId]:t('externalControlUnavailable')}))});
  ...
  function updateJob(sourceId: string, job: ExternalJob) { setJobs((value) => value[sourceId]?.job_id !== job.job_id || value[sourceId] === job ? value : { ...value, [sourceId]: job }); if(['completed','partial'].includes(job.state)&&!importedJobs.has(job.job_id)){importedJobs.add(job.job_id);importJob.mutate({sourceId,jobId:job.job_id});} }
