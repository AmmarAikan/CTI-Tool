...
export function Sources() {
  ...
  function handleTerminal(sourceId:string,job:ExternalJob){if(!TERMINAL_STATES.includes(job.state))return;forgetActiveExternalJob(sourceId==='all-enabled'?'sources-all':`sources-${sourceId}`,job.job_id);void queryClient.invalidateQueries({queryKey:externalQueryKeys.reviews()});if(['completed','partial'].includes(job.state)&&!importedJobs.has(job.job_id)){importedJobs.add(job.job_id);importJob.mutate({sourceId,jobId:job.job_id})}}
