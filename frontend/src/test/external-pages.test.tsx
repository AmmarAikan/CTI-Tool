...
describe('external reviews states', () => {
  ...
  it('retries a processing_failed approval with the identical record hash and no new decision fields',async()=>{
    sessionStorage.setItem('cti_language','en');sessionStorage.setItem('cti_access_token','token');const record={record_id:'manual-json-retry-1234567890',title:'Retry exact export',source_type:'manual_url',review_reason:'classification_review',review_reasons:['classification_review'],classification_label:'cti_related',privacy_status:'reviewed',collected_at:null,published:null,content_sha256:`sha256:${'f'.repeat(64)}`,review_version:'external_review_v2',summary:'Safe summary',excerpt:'Sanitized',source:'Manual JSON',category:'manual',status:'processing_failed'};let completed=false;
    const fetchMock=vi.spyOn(globalThis,'fetch').mockImplementation((input,init)=>{const path=String(input);if(path.endsWith('/auth/me'))return response({id:'u',username:'analyst',role:'analyst',is_active:true});if(path.endsWith('/reviews/latest'))return response({run_id:'run-1',records:completed?[]:[record]});if(path.endsWith(`/reviews/${record.record_id}/decision`)&&init?.method==='POST'){completed=true;return response({schema_version:'1.0',record_id:record.record_id,content_sha256:record.content_sha256,decision:'approved',reason:null,decided_at:'2026-09-26T00:00:00Z',export_run_id:'ext-review-retry-1',processing_state:'completed',retryable:false})}return response({},404)});
    vi.spyOn(window,'confirm').mockReturnValue(true);renderWithProviders(<Reviews/>);await userEvent.click(await screen.findByRole('button',{name:'Retry'}));expect(await screen.findByRole('status')).toHaveTextContent('Record approved');const call=fetchMock.mock.calls.find(([input])=>String(input).endsWith(`/reviews/${record.record_id}/decision`));expect(call?.[1]?.body).toBe(JSON.stringify({expected_content_sha256:record.content_sha256,decision:'approved',reason:null}));
  });
});
