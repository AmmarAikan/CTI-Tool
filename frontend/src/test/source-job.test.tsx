...
describe('single external source job', () => {
  it('accepts the real completed RSS contract in Sources and imports its exact job once',async()=>{const real={schema_version:'1.0',job_id:'job-46de1234567890abcdef',command_id:'cmd-46de1234567890abcdef',state:'completed',created_at:'2026-09-25T10:00:00Z',updated_at:'2026-09-25T10:01:00Z',progress:{},result:{status:'completed',source_id:'source-one',accepted_records:15,review_records:0,rejected_records:0,skipped_records:0,error_count:0,sources:{'source-one':{status:'completed',collection_method:'rss',accepted_records:15,review_records:0,rejected_records:0,skipped_records:0,error_count:0}},export:{status:'completed',run_id:'ext-46de1234567890abcdef',dataset_sha256:'a'.repeat(64),accepted_records:15,review_records:0}},error:null};const fetchMock=mockRole('analyst',(path)=>path.endsWith('/sources/source-one/jobs')?response(real):path.endsWith('/jobs/import')?response({run_id:'central-46de123456',status:'completed',imported:15,unchanged:0,updated:0,failed:0,total:15}):response(real));vi.spyOn(window,'confirm').mockReturnValue(true);renderWithProviders(<Sources/>);await userEvent.click(await screen.findByRole('button',{name:`تشغيل ${enabled.name}`}));expect(await screen.findByText('15')).toBeInTheDocument();await waitFor(()=>expect(fetchMock.mock.calls.filter(([input])=>String(input).endsWith('/jobs/import'))).toHaveLength(1));expect(JSON.parse(String(fetchMock.mock.calls.find(([input])=>String(input).endsWith('/jobs/import'))?.[1]?.body)).job_id).toBe(real.job_id)});
