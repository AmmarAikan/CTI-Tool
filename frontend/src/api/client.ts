...
export interface LatestReviews { run_id: string; records: ReviewRecord[]; malformed_records?: number; }
...
function parseReviews(value: unknown): LatestReviews {
  if (!isPlainObject(value) || Object.keys(value).length !== 2 || typeof value.run_id !== 'string' || value.run_id.length > 200 || !Array.isArray(value.records) || value.records.length > 100) throw new ApiError(502, 'invalid_response', 'Invalid reviews response');
  ...
  return { run_id: value.run_id, records, ...(malformed ? { malformed_records: malformed } : {}) };
}
...
export const api = {
  ...
  latestExternalReviews: async (signal?:AbortSignal) => parseReviews(await request<unknown>('/integrations/external-control/reviews/latest',{signal})),
