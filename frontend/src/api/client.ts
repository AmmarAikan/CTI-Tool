...
function parseSources(value: unknown): Source[] {
  ...
  return value.map((item) => {
    ...
    if (typeof source.source_id !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,199}$/.test(source.source_id) || typeof source.name !== 'string' || !source.name || source.name.length > 200 || typeof source.source_type !== 'string' || !source.source_type || source.source_type.length > 80 || typeof source.status !== 'string' || !['enabled', 'disabled', 'pending_review', 'requires_configuration'].includes(source.status) || !isPlainObject(source.metadata) || Object.keys(source.metadata).some((key) => !['category', 'method'].includes(key)) || Object.values(source.metadata).some((entry) => typeof entry !== 'string' || !entry || entry.length > 100)) throw new ApiError(502, 'invalid_response', 'Invalid sources response');
