...
export interface ExternalPullResult extends Omit<PullResult,'pipeline'> { pipeline:'external'; }
export interface AcceptedSyncResult { run_id:string; status:string; imported:number; unchanged:number; updated:number; failed:number; total:number; }
export interface TrackedManualSource { root_id:string; label:string; method:string; last_checked?:string; active:boolean; origin:string; }
export interface CTIIndicator { id: string; event_id: string; type: string; value: string; confidence: number; source_pipeline: 'external' | 'internal'; severity?: string; first_seen?: string; last_seen?: string; semantic_role: 'external_reference' | 'vulnerability' | 'observable' | 'indicator'; validation_status: 'valid' | 'invalid'; assessment: 'reference' | 'non_actionable' | 'unknown' | 'suspicious' | 'malicious'; assessment_confidence: number; actionable: boolean; evidence_count: number; evidence_providers: string[]; reason_code: string; }
export interface IndicatorSummary { total: number; by_role: Record<string, number>; by_assessment: Record<string, number>; by_validation: Record<string, number>; by_type: Record<string, number>; }
...
export type OutlierFactorKey = 'alert_volume' | 'rule_severity' | 'rule_diversity' | 'failed_actions' | 'credential_attempts';
export interface CTIOutlier { id: string; event_id?: string; started_at: string; ended_at: string; alert_count: number; is_outlier: boolean; anomaly_score: number; detector: string; explanation_method: 'bounded_feature_evidence'; explanation_factors: Array<{ key: OutlierFactorKey; value: number }>; }
...
export interface RunEntity { type:string; value:string; confidence?:number; event_id:string; record_title:string; }
export interface RunIndicator extends RunEntity {}
export interface RunCorrelation { source_event_id:string; target_event_id:string; type:string; score:number; explanation:string; }
export interface RunResults { run_id:string; status:string; imported_documents:number; processed_documents:number; entity_count:number; indicator_count:number; correlation_count:number; review_count:number; rejected_count:number; skipped_count:number; error_count:number; entities:RunEntity[]; indicators:RunIndicator[]; correlations:RunCorrelation[]; }
export interface AcceptedRecord { id:string; title:string; source:string; source_type:string; category?:string; summary:string; published?:string; collected_at?:string; accepted_at:string; processing_state:string; classification?:string; privacy_status?:string; entity_count:number; indicator_count:number; correlation_count:number; content?:string; entities?:RunEntity[]; indicators?:RunIndicator[]; correlations?:RunCorrelation[]; }
export type MLQualityGateKey = 'bert_artifact_present' | 'secondary_artifact_present' | 'bert_test_f1_at_least_0_75' | 'bert_test_accuracy_at_least_0_90' | 'primary_outperforms_secondary_entity_f1' | 'bert_unique_unseen_f1_at_least_0_73' | 'dataset_cross_split_overlap_zero' | 'dataset_label_conflicts_zero' | 'dataset_malformed_lines_zero';
export type MLLimitation = 'saved_metrics_not_live_accuracy' | 'quality_gates_incomplete' | 'primary_unavailable' | 'fallback_unavailable';
export interface MLQualityGate { key: MLQualityGateKey; category: 'artifact' | 'performance' | 'dataset_integrity'; passed: boolean; }
export interface MLStatus { execution_model: 'central_backend'; backend: string; primary_model: string; secondary_model: string; primary_loaded: boolean; secondary_loaded: boolean; quality_gates_passed: boolean; held_out_f1?: number; unique_unseen_f1?: number; runtime_state: 'primary_active' | 'secondary_fallback_active' | 'unavailable'; readiness: 'ready' | 'degraded' | 'unavailable'; inference_scope: 'named_entity_recognition'; inference_evidence: { observed: boolean; input_count: number; chunk_count: number }; metric_scope: 'saved_offline_evaluation'; quality_gates: MLQualityGate[]; limitations: MLLimitation[]; }
...
export interface ReviewRecord { record_id: string; title?: string; source_type?: string; review_reason: string; review_reasons: string[]; classification_label?: string; privacy_status?: string; collected_at?: string; published?: string; content_sha256:string; review_version:string; summary:string; excerpt:string; source:string; category:string; status:'pending'|'approved_processing'|'processing_failed'; }
export interface LatestReviews { run_id: string; records: ReviewRecord[]; total:number; limit:number; offset:number; malformed_records:number; }
...
export interface ManualPreview {
  ...
  detected_type?: 'json_collection';
  collection_path?: string[];
  proposed_mapping?: Record<string, string>;
  validation_warnings?: Array<'explicit_approval_required' | 'empty_collection'>;
...
export interface DarkWebWatch { watch_id:string; keyword:string; keywords?:string[]; match_mode?:'any'|'all'; provider_id?:string; scan_interval_seconds?:number; enabled:boolean; created_at:string; updated_at:string; last_scan_at?:string; last_success_at?:string; result_count:number; new_result_count:number; checkpoint_hash?:string; schedule_enabled?:boolean; next_run_at?:string; last_scheduled_run_at?:string; last_attempt_at?:string; schedule_failure_category?:string; }
export interface DarkWebAlert {alert_id:string;watch_id:string;result_id:string;onion_reference:string;content_sha256:string;alert_type:'new_match'|'content_changed';matched_keywords:string[];created_at:string;read_at?:string}
export interface DarkWebResult { result_id:string; watch_id:string; onion_reference:string; title:string; excerpt:string; provider:string; first_seen_at:string; last_seen_at:string; collected_at?:string; matched_keywords:string[]; status:'new'|'known'; classification_label?:string; classification_confidence?:number; privacy_status:'reviewed'|'review_required'; content_sha256:string; review_reasons:string[]; }
export interface DarkWebDiscoveryWatch extends DarkWebWatch { keywords:string[]; match_mode:'any'|'all'; provider_id:string; scan_interval_seconds:number; }
export interface DarkWebProviderStatus { provider_id:string; enabled:boolean; ready:boolean; through_tor:boolean; }
...
async function request<T>(path: string, options: RequestInit = {}, timeoutMs = DEFAULT_REQUEST_TIMEOUT_MS, allowSessionRecovery = true): Promise<T> {
  ...
  if (response.status === 401 && allowSessionRecovery && path !== '/auth/me' && token) {
    try {
      await request<unknown>('/auth/me', {}, DEFAULT_REQUEST_TIMEOUT_MS, false);
...
function parseExternalPullResult(value:unknown):ExternalPullResult { if(!isPlainObject(value))throw new ApiError(502,'invalid_response','Invalid pull response'); const allowed=['run_id','pipeline','status','collected_count','processed_count','stored_count','failed_count'];exactKeys(value,allowed);if(value.pipeline!=='external')throw new ApiError(502,'invalid_response','Invalid pull response');return {run_id:requiredText(value.run_id,36),pipeline:'external',status:requiredText(value.status,30),collected_count:boundedNumber(value.collected_count),processed_count:boundedNumber(value.processed_count),stored_count:boundedNumber(value.stored_count),failed_count:boundedNumber(value.failed_count)};}
function parseAcceptedSync(value:unknown):AcceptedSyncResult { if(!isPlainObject(value))throw new ApiError(502,'invalid_response','Invalid accepted synchronization response'); exactKeys(value,['run_id','status','imported','unchanged','updated','failed','total']);return {run_id:requiredText(value.run_id,36),status:requiredText(value.status,30),imported:boundedNumber(value.imported),unchanged:boundedNumber(value.unchanged),updated:boundedNumber(value.updated),failed:boundedNumber(value.failed),total:boundedNumber(value.total)}; }
function parseTrackedManualSources(value:unknown):TrackedManualSource[] {if(!isPlainObject(value)||value.schema_version!=='1.0'||!Array.isArray(value.items)||value.items.length>100||Object.keys(value).some(key=>!['schema_version','items'].includes(key)))throw new ApiError(502,'invalid_response','Invalid tracked manual sources response');return value.items.map(item=>{if(!isPlainObject(item))throw new ApiError(502,'invalid_response','Invalid tracked manual source');exactKeys(item,['root_id','label','method','last_checked','active','origin']);if(typeof item.active!=='boolean'||item.last_checked!==null&&item.last_checked!==undefined&&!validTimestamp(item.last_checked))throw new ApiError(502,'invalid_response','Invalid tracked manual source');return {root_id:requiredText(item.root_id,80),label:requiredText(item.label,200),method:requiredText(item.method,40),last_checked:item.last_checked as string|undefined,active:item.active,origin:requiredText(item.origin,40)}})}
...
function requiredText(value: unknown, max: number): string { if (typeof value !== 'string' || !value || value.length > max) throw new ApiError(502, 'invalid_response', 'Invalid intelligence response'); return value; }
...
function parseOutlier(value: unknown): CTIOutlier { if (!isPlainObject(value)) throw new ApiError(502, 'invalid_response', 'Invalid intelligence response'); exactKeys(value, ['id', 'event_id', 'started_at', 'ended_at', 'alert_count', 'is_outlier', 'anomaly_score', 'detector', 'explanation_method', 'explanation_factors']); if (!validIsoTimestamp(value.started_at) || !validIsoTimestamp(value.ended_at) || typeof value.is_outlier !== 'boolean' || value.explanation_method !== 'bounded_feature_evidence' || !Array.isArray(value.explanation_factors) || value.explanation_factors.length > 5) throw new ApiError(502, 'invalid_response', 'Invalid intelligence response'); const keys = ['alert_volume', 'rule_severity', 'rule_diversity', 'failed_actions', 'credential_attempts']; const explanation_factors = value.explanation_factors.map((factor) => { if (!isPlainObject(factor)) throw new ApiError(502, 'invalid_response', 'Invalid intelligence response'); exactKeys(factor, ['key', 'value']); if (!keys.includes(String(factor.key))) throw new ApiError(502, 'invalid_response', 'Invalid intelligence response'); return { key: factor.key as CTIOutlier['explanation_factors'][number]['key'], value: boundedNumber(factor.value) }; }); return { id: requiredText(value.id, 64), event_id: optionalSafeText(value.event_id, 64), started_at: value.started_at, ended_at: value.ended_at, alert_count: boundedNumber(value.alert_count), is_outlier: value.is_outlier, anomaly_score: boundedNumber(value.anomaly_score, -1000, 1000), detector: requiredText(value.detector, 100), explanation_method: 'bounded_feature_evidence', explanation_factors }; }
...
function parseRunEntity(value:unknown):RunEntity { if(!isPlainObject(value)){throw new ApiError(502,'invalid_response','Invalid run results')}; exactKeys(value,['type','value','confidence','event_id','record_title']); return {type:requiredText(value.type,100),value:requiredText(value.value,2048),confidence:value.confidence===null?undefined:boundedNumber(value.confidence,0,1),event_id:requiredText(value.event_id,64),record_title:requiredText(value.record_title,500)}; }
function parseRunCorrelation(value:unknown):RunCorrelation { if(!isPlainObject(value)){throw new ApiError(502,'invalid_response','Invalid run results')}; exactKeys(value,['source_event_id','target_event_id','type','score','explanation']); return {source_event_id:requiredText(value.source_event_id,64),target_event_id:requiredText(value.target_event_id,64),type:requiredText(value.type,50),score:boundedNumber(value.score,0,1),explanation:requiredText(value.explanation,500)}; }
function parseRunResults(value:unknown):RunResults { if(!isPlainObject(value))throw new ApiError(502,'invalid_response','Invalid run results'); const keys=['run_id','status','imported_documents','processed_documents','entity_count','indicator_count','correlation_count','review_count','rejected_count','skipped_count','error_count','entities','indicators','correlations']; exactKeys(value,keys); if(!Array.isArray(value.entities)||value.entities.length>200||!Array.isArray(value.indicators)||value.indicators.length>200||!Array.isArray(value.correlations)||value.correlations.length>200)throw new ApiError(502,'invalid_response','Invalid run results'); const counts=Object.fromEntries(keys.slice(2,11).map(key=>[key,boundedNumber(value[key])])) as Record<string,number>; return {run_id:requiredText(value.run_id,36),status:requiredText(value.status,30),...counts,entities:value.entities.map(parseRunEntity),indicators:value.indicators.map(parseRunEntity),correlations:value.correlations.map(parseRunCorrelation)} as RunResults; }
function parseAcceptedRecord(value:unknown,detail=false):AcceptedRecord { if(!isPlainObject(value))throw new ApiError(502,'invalid_response','Invalid accepted record'); const base=['id','title','source','source_type','category','summary','published','collected_at','accepted_at','processing_state','classification','privacy_status','entity_count','indicator_count','correlation_count']; exactKeys(value,detail?[...base,'content','entities','indicators','correlations']:base); const result:AcceptedRecord={id:requiredText(value.id,64),title:requiredText(value.title,500),source:requiredText(value.source,200),source_type:requiredText(value.source_type,50),category:optionalSafeText(value.category,80),summary:typeof value.summary==='string'&&value.summary.length<=1000?value.summary:(()=>{throw new ApiError(502,'invalid_response','Invalid accepted record')})(),published:optionalTimestamp(value.published),collected_at:optionalTimestamp(value.collected_at),accepted_at:validIsoTimestamp(value.accepted_at)?value.accepted_at:(()=>{throw new ApiError(502,'invalid_response','Invalid accepted record')})(),processing_state:requiredText(value.processing_state,30),classification:optionalSafeText(value.classification,50),privacy_status:optionalSafeText(value.privacy_status,40),entity_count:boundedNumber(value.entity_count),indicator_count:boundedNumber(value.indicator_count),correlation_count:boundedNumber(value.correlation_count)}; if(detail){if(typeof value.content!=='string'||value.content.length>20000||!Array.isArray(value.entities)||!Array.isArray(value.indicators)||!Array.isArray(value.correlations))throw new ApiError(502,'invalid_response','Invalid accepted record'); result.content=value.content;result.entities=value.entities.map(parseRunEntity);result.indicators=value.indicators.map(parseRunEntity);result.correlations=value.correlations.map(parseRunCorrelation)} return result; }
function parseAcceptedRecordPage(value:unknown):Page<AcceptedRecord>{
  if(!isPlainObject(value)||Object.keys(value).some(key=>!pageKeys.includes(key))||!Array.isArray(value.items)||value.items.length>100||!Number.isSafeInteger(value.total)||(value.total as number)<0||!Number.isSafeInteger(value.limit)||(value.limit as number)<1||(value.limit as number)>100||!Number.isSafeInteger(value.offset)||(value.offset as number)<0)throw new ApiError(502,'invalid_response','Invalid accepted records response');
  const items:AcceptedRecord[]=[];let malformed=0;
  for(const item of value.items){try{items.push(parseAcceptedRecord(item))}catch(error){if(error instanceof ApiError&&error.code==='invalid_response'){malformed+=1;continue}throw error}}
  if(value.items.length>0&&items.length===0)throw new ApiError(502,'invalid_response','Invalid accepted records response');
  return {items,total:value.total as number,limit:value.limit as number,offset:value.offset as number,...(malformed?{malformed_items:malformed}:{})};
}
const mlQualityGateKeys: MLQualityGateKey[] = ['bert_artifact_present', 'secondary_artifact_present', 'bert_test_f1_at_least_0_75', 'bert_test_accuracy_at_least_0_90', 'primary_outperforms_secondary_entity_f1', 'bert_unique_unseen_f1_at_least_0_73', 'dataset_cross_split_overlap_zero', 'dataset_label_conflicts_zero', 'dataset_malformed_lines_zero'];
const mlLimitations: MLLimitation[] = ['saved_metrics_not_live_accuracy', 'quality_gates_incomplete', 'primary_unavailable', 'fallback_unavailable'];
function parseMLStatus(value: unknown): MLStatus {
  if (!isPlainObject(value)) throw new ApiError(502, 'invalid_response', 'Invalid ML response');
  exactKeys(value, ['execution_model', 'backend', 'primary_model', 'secondary_model', 'primary_loaded', 'secondary_loaded', 'quality_gates_passed', 'held_out_f1', 'unique_unseen_f1', 'runtime_state', 'readiness', 'inference_scope', 'inference_evidence', 'metric_scope', 'quality_gates', 'limitations']);
  if (!isPlainObject(value.inference_evidence)) throw new ApiError(502, 'invalid_response', 'Invalid ML response');
  exactKeys(value.inference_evidence, ['observed', 'input_count', 'chunk_count']);
  if (value.execution_model !== 'central_backend' || typeof value.primary_loaded !== 'boolean' || typeof value.secondary_loaded !== 'boolean' || typeof value.quality_gates_passed !== 'boolean' || !['primary_active', 'secondary_fallback_active', 'unavailable'].includes(String(value.runtime_state)) || !['ready', 'degraded', 'unavailable'].includes(String(value.readiness)) || value.inference_scope !== 'named_entity_recognition' || value.metric_scope !== 'saved_offline_evaluation' || typeof value.inference_evidence.observed !== 'boolean' || !Number.isSafeInteger(value.inference_evidence.input_count) || (value.inference_evidence.input_count as number) < 0 || !Number.isSafeInteger(value.inference_evidence.chunk_count) || (value.inference_evidence.chunk_count as number) < 0 || !Array.isArray(value.quality_gates) || value.quality_gates.length !== mlQualityGateKeys.length || !Array.isArray(value.limitations) || value.limitations.length > mlLimitations.length) throw new ApiError(502, 'invalid_response', 'Invalid ML response');
  const quality_gates = value.quality_gates.map((gate): MLQualityGate => {
    if (!isPlainObject(gate)) throw new ApiError(502, 'invalid_response', 'Invalid ML response');
    exactKeys(gate, ['key', 'category', 'passed']);
    if (!mlQualityGateKeys.includes(gate.key as MLQualityGateKey) || !['artifact', 'performance', 'dataset_integrity'].includes(String(gate.category)) || typeof gate.passed !== 'boolean') throw new ApiError(502, 'invalid_response', 'Invalid ML response');
    return { key: gate.key as MLQualityGateKey, category: gate.category as MLQualityGate['category'], passed: gate.passed };
  });
  const gateKeys = new Set(quality_gates.map((gate) => gate.key));
  if (gateKeys.size !== mlQualityGateKeys.length || mlQualityGateKeys.some((key) => !gateKeys.has(key)) || quality_gates.every((gate) => gate.passed) !== value.quality_gates_passed) throw new ApiError(502, 'invalid_response', 'Invalid ML response');
  const limitations = value.limitations.map((item) => { if (!mlLimitations.includes(item as MLLimitation)) throw new ApiError(502, 'invalid_response', 'Invalid ML response'); return item as MLLimitation; });
  if (new Set(limitations).size !== limitations.length) throw new ApiError(502, 'invalid_response', 'Invalid ML response');
  const inference_evidence = { observed: value.inference_evidence.observed, input_count: value.inference_evidence.input_count as number, chunk_count: value.inference_evidence.chunk_count as number };
  if (inference_evidence.observed !== (inference_evidence.input_count > 0)) throw new ApiError(502, 'invalid_response', 'Invalid ML response');
  return { execution_model: 'central_backend', backend: requiredText(value.backend, 50), primary_model: requiredText(value.primary_model, 80), secondary_model: requiredText(value.secondary_model, 80), primary_loaded: value.primary_loaded, secondary_loaded: value.secondary_loaded, quality_gates_passed: value.quality_gates_passed, held_out_f1: value.held_out_f1 === null ? undefined : boundedNumber(value.held_out_f1, 0, 1), unique_unseen_f1: value.unique_unseen_f1 === null ? undefined : boundedNumber(value.unique_unseen_f1, 0, 1), runtime_state: value.runtime_state as MLStatus['runtime_state'], readiness: value.readiness as MLStatus['readiness'], inference_scope: 'named_entity_recognition', inference_evidence, metric_scope: 'saved_offline_evaluation', quality_gates, limitations };
}
...
function parseSources(value: unknown): Source[] {
  ...
  return value.map((item) => {
    ...
    if (typeof source.source_id !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,199}$/.test(source.source_id) || typeof source.name !== 'string' || !source.name || source.name.length > 200 || typeof source.source_type !== 'string' || !source.source_type || source.source_type.length > 80 || typeof source.status !== 'string' || !['enabled', 'ready', 'disabled', 'pending_review', 'requires_configuration', 'temporarily_unavailable', 'unsupported_configuration'].includes(source.status) || !isPlainObject(source.metadata) || Object.keys(source.metadata).some((key) => !['category', 'method', 'transport', 'limitation', 'origin'].includes(key)) || Object.values(source.metadata).some((entry) => typeof entry !== 'string' || !entry || entry.length > 100)) throw new ApiError(502, 'invalid_response', 'Invalid sources response');
...
const DISCOVERY_COUNT_KEYS = ['discovered', 'rejected', 'unreachable', 'verified', 'matched', 'new', 'unchanged', 'privacy_blocked', 'errors'] as const;
const EXTERNAL_COLLECTION_METHODS = ['cert','configured_onion_get','dark_web','hackernews','official_csaf','official_listing','official_rss','reddit','reddit_browser_fallback','reddit_oauth','reddit_public_rss','rss','telegram','vulnerability'] as const;
...
function parseExternalJob(value: unknown): ExternalJob {
  ...
  if (body.result) {
    const resultKeys = ['status', 'scope', 'run_id', 'source_id', 'source_count', 'registered_source_count', 'manual_source_count', 'records_created', 'records_updated', ...JOB_COUNT_KEYS, ...DISCOVERY_COUNT_KEYS, 'force', 'sources', 'manual_sources', 'export'];
    ...
    for (const key of DISCOVERY_COUNT_KEYS) {
      const amount = body.result[key];
      if (amount !== undefined) {
        if (!Number.isSafeInteger(amount) || (amount as number) < 0) throw new ApiError(502, 'invalid_response', 'Invalid external job response');
        counts[key] = amount as number;
...
function parseJobSources(value: unknown, target: ExternalJob['sources']) { if (!isPlainObject(value) || Object.keys(value).length > 100) throw new ApiError(502, 'invalid_response', 'Invalid external job response'); for (const [id, item] of Object.entries(value)) { const diagnosticKeys=['status','collection_method','failure_categories','collection_stage','exception_class','retryable']; if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,199}$/.test(id) || !isPlainObject(item) || typeof item.status !== 'string' || !item.status || item.status.length > 40 || Object.keys(item).some((key) => !diagnosticKeys.includes(key) && !JOB_COUNT_KEYS.includes(key as typeof JOB_COUNT_KEYS[number])) || (item.collection_method !== undefined && !EXTERNAL_COLLECTION_METHODS.includes(String(item.collection_method) as typeof EXTERNAL_COLLECTION_METHODS[number])) || (item.collection_stage !== undefined && !/^[a-z][a-z0-9_]{0,63}$/.test(String(item.collection_stage))) || (item.exception_class !== undefined && !/^[A-Za-z][A-Za-z0-9_]{0,99}$/.test(String(item.exception_class))) || (item.retryable !== undefined && typeof item.retryable !== 'boolean')) throw new ApiError(502, 'invalid_response', 'Invalid external job response'); const sourceCounts: ExternalJob['counts'] = {}; for (const key of JOB_COUNT_KEYS) if (item[key] !== undefined) { if (!Number.isSafeInteger(item[key]) || (item[key] as number) < 0) throw new ApiError(502, 'invalid_response', 'Invalid external job response'); sourceCounts[key] = item[key] as number; } let failureCategories: Record<string, number> | undefined; if (item.failure_categories !== undefined) { if (!isPlainObject(item.failure_categories) || Object.keys(item.failure_categories).length > 20 || Object.entries(item.failure_categories).some(([key,count]) => !/^[a-z][a-z0-9_]{0,63}$/.test(key) || !Number.isSafeInteger(count) || (count as number) < 1 || (count as number) > 10000)) throw new ApiError(502, 'invalid_response', 'Invalid external job response'); failureCategories=item.failure_categories as Record<string,number>; } target[id] = { status: sanitizeError(item.status), counts: sourceCounts, collection_method: item.collection_method as string | undefined, failure_categories: failureCategories }; } }
...
function parseReviews(value: unknown): LatestReviews {
  const pageKeys=['run_id','records','total','limit','offset','malformed_records'];
  if (!isPlainObject(value) || Object.keys(value).length !== pageKeys.length || Object.keys(value).some(key=>!pageKeys.includes(key)) || typeof value.run_id !== 'string' || value.run_id.length > 200 || !Array.isArray(value.records) || value.records.length > 50 || !Number.isSafeInteger(value.total)||(value.total as number)<0||!Number.isSafeInteger(value.limit)||(value.limit as number)<1||(value.limit as number)>50||!Number.isSafeInteger(value.offset)||(value.offset as number)<0||!Number.isSafeInteger(value.malformed_records)||(value.malformed_records as number)<0) throw new ApiError(502, 'invalid_response', 'Invalid reviews response');
  const allowed = ['record_id', 'title', 'source_type', 'review_reason', 'review_reasons', 'classification_label', 'privacy_status', 'collected_at', 'published','content_sha256','review_version','summary','excerpt','source','category','status'];
  const records: ReviewRecord[] = []; let malformed = 0;
  for (const item of value.records) { try {
    if (!isPlainObject(item) || Object.keys(item).some((key) => !allowed.includes(key)) || typeof item.record_id !== 'string' || !item.record_id || item.record_id.length > 200
      ...
      || (item.collected_at !== null && item.collected_at !== undefined && !validTimestamp(item.collected_at))
      || typeof item.content_sha256!=='string'||!/^sha256:[0-9a-f]{64}$/.test(item.content_sha256)||typeof item.review_version!=='string'||!item.review_version||item.review_version.length>40||typeof item.summary!=='string'||item.summary.length>500||typeof item.excerpt!=='string'||!item.excerpt||item.excerpt.length>2000||typeof item.source!=='string'||!item.source||item.source.length>200||typeof item.category!=='string'||!item.category||item.category.length>80||!['pending','approved_processing','processing_failed'].includes(String(item.status))) throw new ApiError(502, 'invalid_response', 'Invalid reviews response');
    if (item.classification_label !== null && item.classification_label !== undefined && typeof item.classification_label !== 'string') throw new ApiError(502, 'invalid_response', 'Invalid reviews response');
    records.push({ record_id: item.record_id, title: optionalText(item.title, 300), source_type: optionalText(item.source_type, 100), review_reason: sanitizeError(item.review_reason), review_reasons: item.review_reasons.map((reason) => sanitizeError(reason as string)), classification_label: optionalText(item.classification_label, 100), privacy_status: optionalText(item.privacy_status, 100), collected_at: item.collected_at as string | undefined, published: normalizePublishedDate(item.published),content_sha256:item.content_sha256,review_version:item.review_version,summary:sanitizeError(item.summary),excerpt:sanitizeError(item.excerpt),source:sanitizeError(item.source),category:item.category,status:item.status as ReviewRecord['status'] });
  } catch (error) { if (error instanceof ApiError && error.code === 'invalid_response') { malformed += 1; continue; } throw error; } }
  if (value.records.length > 0 && records.length === 0) throw new ApiError(502, 'invalid_response', 'Invalid reviews response');
  return { run_id: value.run_id, records, total:value.total as number, limit:value.limit as number, offset:value.offset as number, malformed_records:(value.malformed_records as number)+malformed };
}
...
function parseManualPreview(value: unknown): ManualPreview {
  ...
  const jsonKeys = ['detected_type', 'collection_path', 'proposed_mapping', 'validation_warnings', 'approval_required'];
  if (Object.keys(value).some((key) => !required.includes(key) && !nullable.includes(key) && !jsonKeys.includes(key)) || required.some((key) => !(key in value))
  ...
  const presentJson = jsonKeys.filter((key) => key in value);
  if (presentJson.length && (presentJson.length !== jsonKeys.length || value.detected_type !== 'json_collection'
    || value.approval_required !== true || !Array.isArray(value.collection_path) || value.collection_path.length > 4
    || value.collection_path.some((part) => typeof part !== 'string' || !/^[A-Za-z][A-Za-z0-9_]{0,79}$/.test(part))
    || !isPlainObject(value.proposed_mapping) || Object.keys(value.proposed_mapping).length > 9
    || Object.entries(value.proposed_mapping).some(([key, field]) => !/^(id|title|content|summary|published|updated|link)_field$/.test(key) || typeof field !== 'string' || !/^[A-Za-z][A-Za-z0-9_]{0,79}$/.test(field))
    || !Array.isArray(value.validation_warnings) || value.validation_warnings.length > 4
    || value.validation_warnings.some((warning) => !['explicit_approval_required', 'empty_collection'].includes(String(warning))))) {
    throw new ApiError(502, 'invalid_response', 'Invalid manual preview response');
  }
...
function parseDarkWebWatch(value:unknown):DarkWebWatch { if(!isPlainObject(value)){throw new ApiError(502,'invalid_response','Invalid dark web response');} const keys=['watch_id','keyword','keywords','match_mode','provider_id','scan_interval_seconds','enabled','created_at','updated_at','last_scan_at','last_success_at','result_count','new_result_count','checkpoint_hash','schedule_enabled','next_run_at','last_scheduled_run_at','last_attempt_at','schedule_failure_category']; exactKeys(value,keys); if(typeof value.enabled!=='boolean'||typeof value.schedule_enabled!=='boolean'||!validIsoTimestamp(value.created_at)||!validIsoTimestamp(value.updated_at))throw new ApiError(502,'invalid_response','Invalid dark web response'); const watchId=requiredText(value.watch_id,40),keyword=requiredText(value.keyword,100),checkpoint=optionalSafeText(value.checkpoint_hash,64),keywords=value.keywords;if(!/^[A-Za-z0-9][A-Za-z0-9._:-]{9,39}$/.test(watchId)||/https?:\/\/|\.onion\b/i.test(keyword)||(checkpoint!==undefined&&!/^[0-9a-f]{64}$/.test(checkpoint))||(keywords!==null&&keywords!==undefined&&(!Array.isArray(keywords)||keywords.length<1||keywords.length>10||keywords.some(v=>typeof v!=='string'||v.length<2||v.length>100)))||(value.match_mode!==null&&value.match_mode!==undefined&&!['any','all'].includes(String(value.match_mode))))throw new ApiError(502,'invalid_response','Invalid dark web response'); return {watch_id:watchId,keyword,keywords:keywords as string[]|undefined,match_mode:value.match_mode as 'any'|'all'|undefined,provider_id:value.provider_id as string|undefined,scan_interval_seconds:value.scan_interval_seconds as number|undefined,enabled:value.enabled,created_at:value.created_at,updated_at:value.updated_at,last_scan_at:optionalTimestamp(value.last_scan_at),last_success_at:optionalTimestamp(value.last_success_at),result_count:boundedNumber(value.result_count),new_result_count:boundedNumber(value.new_result_count),checkpoint_hash:checkpoint,schedule_enabled:value.schedule_enabled,next_run_at:optionalTimestamp(value.next_run_at),last_scheduled_run_at:optionalTimestamp(value.last_scheduled_run_at),last_attempt_at:optionalTimestamp(value.last_attempt_at),schedule_failure_category:optionalSafeText(value.schedule_failure_category,64)}; }
...
function parseDarkWebResults(value:unknown):Page<DarkWebResult>{if(!isPlainObject(value)||value.schema_version!=='1.0'||Object.keys(value).some(k=>!['schema_version','items','total','limit','offset'].includes(k)))throw new ApiError(502,'invalid_response','Invalid dark web response'); const page={items:value.items,total:value.total,limit:value.limit,offset:value.offset};return parsePage(page,(item)=>{if(!isPlainObject(item))throw new ApiError(502,'invalid_response','Invalid dark web response'); const keys=['result_id','watch_id','onion_reference','title','excerpt','provider','first_seen_at','last_seen_at','collected_at','matched_keywords','status','classification_label','classification_confidence','privacy_status','content_sha256','review_reasons'];exactKeys(item,keys);const raw=JSON.stringify(item),resultId=requiredText(item.result_id,36),watchId=requiredText(item.watch_id,40),reference=requiredText(item.onion_reference,74),title=requiredText(item.title,200),excerpt=requiredText(item.excerpt,240),provider=requiredText(item.provider,100),contentHash=requiredText(item.content_sha256,64),matched=item.matched_keywords;if(/https?:\/\/|\.onion\b|(?:token|password|secret|authorization|cookie|api[_-]?key)\s*[=:]/i.test(raw)||!/^dwr-[0-9a-f]{32}$/.test(resultId)||!/^[A-Za-z0-9][A-Za-z0-9._:-]{9,39}$/.test(watchId)||!/^onion-ref:[0-9a-f]{12,64}$/.test(reference)||!/^[0-9a-f]{64}$/.test(contentHash)||!Array.isArray(matched)||matched.length>10||matched.some(v=>typeof v!=='string'||v.length<2||v.length>100)||!['new','known'].includes(String(item.status))||!['reviewed','review_required'].includes(String(item.privacy_status))||!Array.isArray(item.review_reasons)||item.review_reasons.length>10||item.review_reasons.some(reason=>typeof reason!=='string'||reason.length>80)||!validIsoTimestamp(item.first_seen_at)||!validIsoTimestamp(item.last_seen_at)||(item.collected_at!==null&&item.collected_at!==undefined&&!validIsoTimestamp(item.collected_at))||(item.classification_label!==null&&item.classification_label!==undefined&&(typeof item.classification_label!=='string'||item.classification_label.length>80))||(item.classification_confidence!==null&&item.classification_confidence!==undefined&&(typeof item.classification_confidence!=='number'||item.classification_confidence<0||item.classification_confidence>1)))throw new ApiError(502,'invalid_response','Invalid dark web response');return {result_id:resultId,watch_id:watchId,onion_reference:reference,title,excerpt,provider,first_seen_at:item.first_seen_at,last_seen_at:item.last_seen_at,collected_at:item.collected_at as string|undefined,matched_keywords:matched as string[],status:item.status as DarkWebResult['status'],classification_label:item.classification_label as string|undefined,classification_confidence:item.classification_confidence as number|undefined,privacy_status:item.privacy_status as DarkWebResult['privacy_status'],content_sha256:contentHash,review_reasons:item.review_reasons as string[]};});}
function parseDiscoveryWatch(value:unknown):DarkWebDiscoveryWatch{if(!isPlainObject(value)||!Array.isArray(value.keywords)||value.keywords.length<1||value.keywords.length>10||value.keywords.some(v=>typeof v!=='string'||v.length<2||v.length>100)||!['any','all'].includes(String(value.match_mode))||typeof value.provider_id!=='string'||!Number.isSafeInteger(value.scan_interval_seconds))throw new ApiError(502,'invalid_response','Invalid dark web response');const base=parseDarkWebWatch(Object.fromEntries(Object.entries(value).filter(([key])=>!['keywords','match_mode','provider_id','scan_interval_seconds'].includes(key))));return {...base,keywords:value.keywords as string[],match_mode:value.match_mode as 'any'|'all',provider_id:value.provider_id,scan_interval_seconds:value.scan_interval_seconds as number};}
function parseProviderStatuses(value:unknown):DarkWebProviderStatus[]{if(!Array.isArray(value)||value.length>10||value.some(item=>!isPlainObject(item)||Object.keys(item).some(k=>!['provider_id','enabled','ready','through_tor'].includes(k))||typeof item.provider_id!=='string'||['enabled','ready','through_tor'].some(k=>typeof item[k]!=='boolean')))throw new ApiError(502,'invalid_response','Invalid dark web response');return value as DarkWebProviderStatus[];}
function parseDiscoveredSource(value:unknown):DarkWebDiscoveredSource{const keys=['source_id','watch_id','onion_reference','enabled','created_at','updated_at'];if(!isPlainObject(value)||Object.keys(value).length!==keys.length||Object.keys(value).some(k=>!keys.includes(k))||!/^dws-[0-9a-f]{24}$/.test(String(value.source_id))||!/^[A-Za-z0-9][A-Za-z0-9._:-]{9,39}$/.test(String(value.watch_id))||!/^onion-ref:[0-9a-f]{64}$/.test(String(value.onion_reference))||typeof value.enabled!=='boolean'||!validIsoTimestamp(value.created_at)||!validIsoTimestamp(value.updated_at))throw new ApiError(502,'invalid_response','Invalid dark web response');return value as unknown as DarkWebDiscoveredSource;}
function parseDarkWebAlerts(value:unknown):Page<DarkWebAlert>&{unread:number}{if(!isPlainObject(value)||value.schema_version!=='1.0'||!Array.isArray(value.items)||value.items.length>100||typeof value.unread!=='number')throw new ApiError(502,'invalid_response','Invalid dark web alert response');const page=parsePage({items:value.items,total:value.total,limit:value.limit,offset:value.offset},item=>{if(!isPlainObject(item)||!/^dwa-[0-9a-f]{32}$/.test(String(item.alert_id))||!/^dwr-[0-9a-f]{32}$/.test(String(item.result_id))||!/^onion-ref:[0-9a-f]{12,64}$/.test(String(item.onion_reference))||!['new_match','content_changed'].includes(String(item.alert_type))||!Array.isArray(item.matched_keywords)||!validIsoTimestamp(item.created_at))throw new ApiError(502,'invalid_response','Invalid dark web alert response');return item as unknown as DarkWebAlert});return {...page,unread:value.unread}}
...
export const api = {
  ...
  pullExternal:async(signal?:AbortSignal)=>parseExternalPullResult(await request<unknown>('/integrations/external-feed/pull',{method:'POST',signal},INTERNAL_PULL_TIMEOUT_MS)),
  ...
  analysisRunResults: async(id:string,signal?:AbortSignal)=>{const result=parseRunResults(await request<unknown>(`/intelligence/runs/${encodeURIComponent(id)}/results`,{signal}));if(result.run_id!==id)throw new ApiError(502,'invalid_response','Run identity mismatch');return result;},
  acceptedRecords: async(limit=25,offset=0,filters:{search?:string;source?:string;source_type?:string;processing_state?:string}={},signal?:AbortSignal)=>{const params=new URLSearchParams({limit:String(limit),offset:String(offset)});Object.entries(filters).forEach(([key,value])=>{if(value)params.set(key,value)});return parseAcceptedRecordPage(await request<unknown>(`/intelligence/accepted-records?${params}`,{signal}));},
  syncAcceptedRecords: async(signal?:AbortSignal)=>parseAcceptedSync(await request<unknown>('/integrations/external-control/exports/sync',{method:'POST',signal},INTERNAL_PULL_TIMEOUT_MS)),
  importExternalJob: async(jobId:string,signal?:AbortSignal)=>parseAcceptedSync(await request<unknown>('/integrations/external-control/jobs/import',{method:'POST',body:JSON.stringify({job_id:jobId}),signal},INTERNAL_PULL_TIMEOUT_MS)),
  acceptedRecord: async(id:string,signal?:AbortSignal)=>{const value=parseAcceptedRecord(await request<unknown>(`/intelligence/accepted-records/${encodeURIComponent(id)}`,{signal}),true);if(value.id!==id)throw new ApiError(502,'invalid_response','Accepted record identity mismatch');return value;},
  ...
  enableExternalSource: async(sourceId:string)=>parseSources([await request<unknown>(`/integrations/external-control/sources/${encodeURIComponent(sourceId)}/enable`,{method:'POST'})])[0],
  disableExternalSource: async(sourceId:string)=>parseSources([await request<unknown>(`/integrations/external-control/sources/${encodeURIComponent(sourceId)}/disable`,{method:'POST'})])[0],
  deleteExternalSource: async(sourceId:string)=>{await request<unknown>(`/integrations/external-control/sources/${encodeURIComponent(sourceId)}`,{method:'DELETE'});},
  ...
  trackedManualSources: async()=>parseTrackedManualSources(await request<unknown>('/integrations/external-control/manual-sources/tracked')),
  recheckManualSource: async (rootId: string, force=false, signal?: AbortSignal) => parseExternalJob(await request<unknown>('/integrations/external-control/manual-sources/recheck', { method: 'POST', body: JSON.stringify({ root_id:rootId, force }), signal })),
  ...
  latestExternalReviews: async (limit=20,offset=0,signal?:AbortSignal) => parseReviews(await request<unknown>(limit===20&&offset===0?'/integrations/external-control/reviews/latest':`/integrations/external-control/reviews/latest?limit=${limit}&offset=${offset}`,{signal})),
  decideExternalReview:async(record:ReviewRecord,decision:'approved'|'rejected',reason?:'not_relevant'|'duplicate'|'privacy_risk'|'low_quality',signal?:AbortSignal)=>{const value=await request<unknown>(`/integrations/external-control/reviews/${encodeURIComponent(record.record_id)}/decision`,{method:'POST',body:JSON.stringify({expected_content_sha256:record.content_sha256,decision,reason:reason||null}),signal});const keys=['schema_version','record_id','content_sha256','decision','reason','decided_at','export_run_id','processing_state','retryable'];if(!isPlainObject(value)||Object.keys(value).length!==keys.length||Object.keys(value).some(key=>!keys.includes(key))||value.schema_version!=='1.0'||value.record_id!==record.record_id||value.content_sha256!==record.content_sha256||value.decision!==decision||value.reason!==(reason||null)||!validTimestamp(value.decided_at)||value.processing_state!=='completed'||value.retryable!==false||(decision==='approved'&&(typeof value.export_run_id!=='string'||value.export_run_id.length<8))||(decision==='rejected'&&value.export_run_id!==null))throw new ApiError(502,'invalid_response','Invalid review decision response');return value;},
  externalJobs: async () => parseJobHistory(await request<unknown>('/integrations/external-control/jobs?limit=50')),
  cancelExternalJob: async (jobId: string) => { const job = parseExternalJob(await request<unknown>(`/integrations/external-control/jobs/${encodeURIComponent(jobId)}/cancel`, { method: 'POST' })); if (job.job_id !== jobId) throw new ApiError(502, 'invalid_response', 'External job identity mismatch'); return job; },
  ...
  patchDarkWebSchedule:async(watchId:string,enabled:boolean,intervalSeconds:3600|21600|43200|86400)=>parseDarkWebWatch(await request<unknown>(`/dark-web/watches/${encodeURIComponent(watchId)}/schedule`,{method:'PATCH',body:JSON.stringify({enabled,interval_seconds:intervalSeconds})})),
  ...
  darkWebResults: async(watchId:string,limit=25,offset=0)=>{const page=parseDarkWebResults(await request<unknown>(`/dark-web/watches/${encodeURIComponent(watchId)}/results?limit=${limit}&offset=${offset}`));if(page.items.some(item=>item.watch_id!==watchId))throw new ApiError(502,'invalid_response','Dark web result watch identity mismatch');return page;},
  darkWebDiscoveryProviders:async()=>parseProviderStatuses(await request<unknown>('/dark-web/discovery/providers')),
  createDarkWebDiscoveryWatch:async(keywords:string[],matchMode:'any'|'all',providerId:string)=>parseDiscoveryWatch(await request<unknown>('/dark-web/discovery/watches',{method:'POST',body:JSON.stringify({keywords,match_mode:matchMode,provider_id:providerId,scan_interval_seconds:3600})})),
  promoteDarkWebResult:async(watchId:string,resultId:string,contentSha256:string)=>parseDiscoveredSource(await request<unknown>(`/dark-web/watches/${encodeURIComponent(watchId)}/results/${encodeURIComponent(resultId)}/promote`,{method:'POST',headers:{'Idempotency-Key':crypto.randomUUID()},body:JSON.stringify({expected_content_sha256:contentSha256})})),
  darkWebDiscoveredSources:async(watchId:string)=>{const value=await request<unknown>(`/dark-web/watches/${encodeURIComponent(watchId)}/discovered-sources`);if(!Array.isArray(value)||value.length>100)throw new ApiError(502,'invalid_response','Invalid dark web response');return value.map(parseDiscoveredSource);},
  patchDarkWebDiscoveredSource:async(sourceId:string,enabled:boolean)=>parseDiscoveredSource(await request<unknown>(`/dark-web/discovered-sources/${encodeURIComponent(sourceId)}`,{method:'PATCH',body:JSON.stringify({enabled})})),
  darkWebAlerts:async(watchId?:string,limit=25,offset=0)=>parseDarkWebAlerts(await request<unknown>(`/dark-web/alerts?limit=${limit}&offset=${offset}${watchId?`&watch_id=${encodeURIComponent(watchId)}`:''}`)),
  markDarkWebAlertRead:async(alertId:string)=>{const page=parseDarkWebAlerts({schema_version:'1.0',items:[await request<unknown>(`/dark-web/alerts/${encodeURIComponent(alertId)}/read`,{method:'PATCH'})],total:1,unread:0,limit:1,offset:0});return page.items[0]},
};
