...
export interface ExternalJob {
  ...
  sources: Record<string, { status: string; counts: ExternalJob['counts']; collection_method?: string }>;
  error?: string;
}
...
export interface DarkWebWatch { watch_id:string; keyword:string; keywords?:string[]; match_mode?:'any'|'all'; provider_id?:string; scan_interval_seconds?:number; enabled:boolean; created_at:string; updated_at:string; last_scan_at?:string; last_success_at?:string; result_count:number; new_result_count:number; checkpoint_hash?:string; }
...
export const api = {
  ...
  recheckManualSource: async (url: string, signal?: AbortSignal) => parseExternalJob(await request<unknown>('/integrations/external-control/manual-sources/recheck', { method: 'POST', body: JSON.stringify({ url, force: true }), signal })),
  ...
  promoteDarkWebResult:async(watchId:string,resultId:string)=>parseDiscoveredSource(await request<unknown>(`/dark-web/watches/${encodeURIComponent(watchId)}/results/${encodeURIComponent(resultId)}/promote`,{method:'POST',headers:{'Idempotency-Key':crypto.randomUUID()}})),
