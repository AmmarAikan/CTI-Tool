...
- A full isolated HTTPS edge -> frontend -> mock backend smoke on a production-like `172.30.200.0/24` Docker network preserved the real test-client IP and removed forged XFF, Forwarded, and Tailscale identity headers. An initial auto-assigned `192.168.0.0/24` network failed closed to the edge peer; future proxy networks must stay inside the reviewed trust range. All temporary containers, network, and test certificates were removed.
