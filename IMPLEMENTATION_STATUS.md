...
| SSRF controls | Release blocker identified | Nine manual URL policy/router tests pass in VPS venv and External Sources Docker, but validated DNS addresses are not pinned to the subsequent requests connection | Obtain External Sources owner approval; pin outbound connections to approved public IPs with original Host/TLS SNI/certificate validation, then test redirects, rebinding, consumers, bounds, and Docker parity |
...
Resolve the documented External Sources DNS-rebinding release blocker after its owner approves the narrow change; keep manual crawler/Telegram compatibility and test in the actual External Sources image. In parallel, validate Tailscale Serve rate-limit fairness and design a public-IP HTTPS edge that preserves tailnet access, without changing production. Follow with supported formal security scan and full Docker/Compose regression before any immutable `/opt` deployment.
